...
# correction des erreur Prtection CSRF et attribut has no collections
from extensions import csrf
import collections
#importation des models
from models import Venue, venue_genre, Artist, artist_genre, Show, Genre, db
...
def venues():
    ...
    for venue in venues:
        citiesState.add( (venue.city, venue.state) )  
    
    citiesState = list(citiesState)
    citiesState.sort(key=itemgetter(1,0))   

    now = datetime.now()    
    for single_citiesState in citiesState:
        v_list = []
        for venue in venues:
            if (venue.city == single_citiesState[0]) and (venue.state == single_citiesState[1]):
        ...
        data.append({
            "city": single_citiesState[0],
            "state": single_citiesState[1],
            "venues": v_list
        })
...
# Affichage d'un element de Venue
...
# Creation de Venue par la methode POST
...
def create_venue_submission():
    ...
    if not form.validate():
        ...
        error = False
        try:
            nouveau_venue = Venue(name=name, city=city, state=state, address=address, phone=phone, \
            ...
            for genre in genres:
                ...
                if get_genre:
                    nouveau_venue.genres.append(get_genre)
                else:
                    nw_genre = Genre(name=genre)
                    db.session.add(nw_genre)
                    nouveau_venue.genres.append(nw_genre)  

            db.session.add(nouveau_venue)
            db.session.commit()
...
# Supprimer un élémeent de venue
